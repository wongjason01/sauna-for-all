...
# Files that must sit at the site root to be found. robots.txt and sitemap.xml
# are generated by build.py from SITE_URL; favicon.ico and site.webmanifest
# are committed as-is (browsers request /favicon.ico by default, even from
# pages that don't declare an icon).
ROOT_FILES = ["robots.txt", "sitemap.xml", "favicon.ico", "site.webmanifest"]
...
def main():
    ...
    for name in ROOT_FILES:
        ...
        if not os.path.isfile(src_path):
            raise RuntimeError(
                f"{name} missing at the repo root -- robots.txt and sitemap.xml "
                "come from running build.py; favicon.ico and site.webmanifest "
                "are committed files.")
        shutil.copyfile(src_path, os.path.join(DIST, name))
        print(f"copied dist/{name}")
